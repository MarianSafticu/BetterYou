import React from "react";
import FriendRequestCard from "./FriendRequestCard";
import FriendRequest from "../../../../models/FriendRequest";
import { connect } from "react-redux";
import { fetchFriendRequestsBegin } from "../../../../redux/actions/actions";
import AppState from "../../../../redux/store/store";

interface IProps {
    friendRequestsList: FriendRequest[];
    fetchFriendRequests: Function;
}

class FriendsListRequests extends React.Component<IProps, {}> {
    constructor(props: IProps) {
        super(props);
        this.state = {
            friendRequestsList: []
        }
    }

    componentDidMount() {
        this.props.fetchFriendRequests();
        console.log("FRIENDREQ: ", this.props.friendRequestsList)
    }

    render() {
        return (
            <div>
                {   
                    this.props.friendRequestsList.map((friendRequest, i) => {
                        return (
                            <div key={i}>
                                <FriendRequestCard
                                    image="data:image/png;base64,"
                                    sender={friendRequest.sender}
                                />
                            </div>
                        )
                    })
                }
            </div>
        )
    }
}


const mapStateToProps = (state: AppState) => {
    return {
        friendRequestsList: state.friendRequests
    }
}

const mapDispatchToProps = (dispatch: any) => {
    return {
        fetchFriendRequests: () => dispatch(fetchFriendRequestsBegin())
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(FriendsListRequests)