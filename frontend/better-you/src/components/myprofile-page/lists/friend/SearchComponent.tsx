import React, { Component, ChangeEvent } from "react";
import { TextField } from "@material-ui/core";
import { fetchUsersBegin } from "../../../../redux/actions/actions";
import { connect } from "react-redux";
import AppState from "../../../../redux/store/store";
import UserInfoDTO from "../../../../models/UserInfoDTO";
import FriendCard from "./FriendCard";
import "../../../../assets/scss/dashboard-page/GoalListStyle.scss";
import "../../../../assets/scss/dashboard-page/DashboardPageStyle.scss";
import "../../../../assets/scss/dashboard-page/FriendPageStyle.scss"

interface IProps {
  users: UserInfoDTO[];
  fetchUsers: Function;
}

class SearchComponent extends Component<IProps, {}> {
  constructor(props: IProps) {
    super(props);
  }
  componentDidMount() {
    if (this.props.users.length === 0)
      this.props.fetchUsers("");
  }
  onChangeUsersList = (event: ChangeEvent<HTMLInputElement>) => {
    this.props.fetchUsers(event.target.value);
  };

  render() {
    return (
      <div className="friend_container">
        <div className="default_list_component">
          <TextField
            onChange={this.onChangeUsersList}
            className="search-input"
            id="search"
            label="Search users"
            defaultValue=""
            variant="filled"
          />
          {this.props.users.map((user, index) =>
            <div key={index}>
              <FriendCard
                image="data:image/png;base64,"
                name={user.profile_name!}
                username={user.username!}
              />
            </div>
          )
          }
        </div>
      </div>
    );
  }
}

const mapStateToProps = (state: AppState) => {
  return {
    users: state.users
  }
}

const mapDispatchToProps = (dispatch: any) => {
  return {
    fetchUsers: (prefix: string) => dispatch(fetchUsersBegin(prefix))
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(SearchComponent);